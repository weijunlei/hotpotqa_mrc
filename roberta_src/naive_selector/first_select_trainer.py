...
from first_select_config import get_config
...
def get_dev_data(args, tokenizer):
    ...
    cached_dev_features_file = '{}/naive_dev_feature_file_{}_{}_{}_{}'.format(args.feature_cache_path,
...
def get_train_data(args, tokenizer):
    """ 获取训练数据 """
    global logger
    cached_train_features_file = '{}/naive_train_feature_file_{}_{}_{}_{}'.format(args.feature_cache_path,
                                                                                  args.bert_model.split('/')[-1],
                                                                                  str(args.max_seq_length),
                                                                                  str(args.doc_stride),
                                                                                  args.log_prefix)
...
def dev_evaluate(args,
    ...
    with torch.no_grad():
        for step, d_batch in enumerate(tqdm(dev_dataloader, desc="Predict Iteration")):
            input_ids, input_mask, segment_ids, example_indices = d_batch
...
def evaluate_result(args, paragraph_results, thread=0.5):
    ...
    for k, v in format_result.items():
        ...
        true_num = 0
        has_result = False
        for predict_result, true_result in zip(predict_results, true_results):
            if predict_result == max_predict_result and not has_result:
                if true_result == 1:
                    has_result = True
                    acc += 1
            if predict_result >= thread and true_result == 1:
                true_num += 1
            if true_result == 1:
                all_true_num += 1
        recall += true_num if true_num <= 2 else 2
        em += 1 if true_num >= 2 else 0
    predict_num = len(format_result) * 1.0
    acc = 1.0 * acc / predict_num
    pre = acc
    recall = 1.0 * recall / all_true_num
    f1 = 2 * (acc * recall) / (acc + recall)
    em = 1.0 * em / predict_num
    return acc, pre, recall, f1, em
