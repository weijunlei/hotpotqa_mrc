    ...
    def forward(self,
        ...
        if len(input_ids.shape) < 2:
            ...
            if start_positions is not None and len(start_positions.shape)<2:
                ...
                sent_lbs = sent_lbs.unsqueeze(0)
                sent_weight = sent_weight.unsqueeze(0)
        sequence_output = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, word_sim_matrix=word_sim_matrix)[0]
        sequence_output = self.dropout(sequence_output)
...
class BertForQuestionAnsweringForwardWithSim(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringForwardWithSim, self).__init__(config)
        self.bert = BertModel(config)
        self.start_logits = nn.Linear(config.hidden_size, 1)
        self.end_logits = nn.Linear(config.hidden_size, 1)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
