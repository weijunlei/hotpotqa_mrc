...
def train_iterator(args,
    ...
    for epoch_idx in trange(int(args.num_train_epochs), desc="Epoch"):
        for start_idx in trange(len(start_idxs), desc="Split Data"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (global_steps + 1) % args.save_model_step == 0 and (
                    ...
                    logger.info(
                        "epoch={:3d},data={:3d},step={:8d},max_acc={:4.8f},precision={:4.8f},em={:4.8f},recall={:4.8f},max_max={:4.8f},loss={:8.3f}".format(
                            epoch_idx, start_idx, global_steps, acc, prec, em, rec, best_predict_acc, total_loss), args)
        ...
        acc, prec, em, rec, total_loss = dev_evaluate(args=args,
                                                      ...
                                                      model_name=args.model_name)
        logger.info(
            "epoch={:3d},data={:3d},step={:8d}, acc={:4.8f},precision={:4.8f},em={:4.8f},recall={:4.8f}, max_predict_acc={:4.8f}, loss={:8.3f}".format(
                epoch_idx, start_idx, global_steps, acc, prec, em, rec, best_predict_acc, total_loss))
