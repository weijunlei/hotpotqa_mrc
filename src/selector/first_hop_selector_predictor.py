...
def run_predict(args):
    ...
    # 从文件中加载模型
    model = models[args.model_name].from_pretrained(args.checkpoint_path)
    ...
    dev_examples = read_hotpotqa_examples(input_file=args.dev_file,
                                          is_training='test')
    all_examples_num = len(dev_examples)
    print("all examples number: {}".format(all_examples_num))
    start = list(range(0, all_examples_num, 215000))
    end = []
    for i in start:
        end.append(i + 215000)
    end[-1] = all_examples_num
    all_results = []
    best_paragraph = {}
    related_sentence = {}
    related_paragraph = {}
    ...
    for idx in range(len(start)):
        truly_examples = dev_examples[start[idx]: end[idx]]
        ...
        print("all truly gotten features: {}".format(len(truly_features)))
    ...
    if not os.path.exists(args.predict_result_path):
        print("make new output dir:{}".format(args.predict_result_path))
