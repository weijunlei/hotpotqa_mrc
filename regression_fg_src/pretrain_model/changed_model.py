...
class ElectraForParagraphClassificationRegression(ElectraModel):
    def __init__(self, config):
        super(ElectraForParagraphClassificationRegression, self).__init__(config)
...
class ElectraForParagraphClassificationTwoRegression(ElectraModel):
    def __init__(self, config):
        super(ElectraForParagraphClassificationTwoRegression, self).__init__(config)
    ...
    def forward(self,
        ...
        if cls_label is not None:
            sent_loss = loss_fn1(sent_logits, cls_label.float())
            sent_loss = (sent_loss * cls_mask.float())
            sent_loss = torch.sum(sent_loss, (-1, -2), keepdim=False)
        else:
            return sent_logits
        # loss = loss_fct(logits.view(-1, self.config.num_labels), cls_label.view(-1))
        return sent_loss, sent_logits  # (loss), scores, (hidden_states), (attentions)
