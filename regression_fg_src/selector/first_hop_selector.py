...
from changed_model import BertForParagraphClassification, BertForRelatedSentence, \
    ...
    ElectraForParagraphClassificationCrossAttention, ElectraForParagraphClassificationRegression
...
def dev_feature_getter(args,
    ...
    d_all_cls_label = torch.tensor([f.cls_label for f in dev_features], dtype=torch.float)
...
def train_iterator(args,
    ...
    for epoch_idx in trange(int(args.num_train_epochs), desc="Epoch"):
        for start_idx in trange(len(start_idxs), desc="Split Data Index: {}".format(epoch_idx)):
            ...
            all_cls_label = torch.tensor([f.cls_label for f in train_features], dtype=torch.float)
