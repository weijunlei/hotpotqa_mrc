...
from qanet import CQAttention, MyQANet
...
class BertForQuestionAnsweringCQAttention(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringCQAttention, self).__init__(config)
        self.bert = BertModel(config)
...
class BertForQuestionAnsweringCQAttentionWithEntity(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringCQAttentionWithEntity, self).__init__(config)
        self.bert = BertModel(config)
        ENTITY_NUM = 20
        ENTITY_DIM = 20
        self.entity_embedder = nn.Embedding(num_embeddings=ENTITY_NUM, embedding_dim=ENTITY_DIM)
        self.cq_attention = MyQANet(config.hidden_size + ENTITY_DIM)
...
class BertForQuestionAnsweringForwardWithEntityTransformer(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringForwardWithEntityTransformer, self).__init__(config)
        self.bert = BertModel(config)
        ENTITY_NUM = 20
        ENTITY_DIM = 8
        self.entity_embedder = nn.Embedding(num_embeddings=ENTITY_NUM, embedding_dim=ENTITY_DIM)
        self.transformer = TransformerLayer(hidden_size=config.hidden_size + ENTITY_DIM,
                                            head_num=1,
                                            dropout=config.hidden_dropout_prob,
                                            feedforward_size=(config.hidden_size + ENTITY_DIM) * 3)
        self.start_logits = nn.Linear(config.hidden_size + ENTITY_DIM, 1)
        self.end_logits = nn.Linear(config.hidden_size + ENTITY_DIM, 1)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
